...
# Create hospital recommendation agent using Agno framework
agent = Agent(
    model=OpenAIChat(id="gpt-4.1-nano", temperature=0.4),
    # model=Groq(id="llama-3.3-70b-versatile", temperature=0.4),
...
if __name__ == "__main__":
    # Option 1: Show detailed results of each step
    sample_data = create_sample_data()
    response = agent.print_response(sample_data)
    
    # # Option 2: Extract and print only the recommendations
    # response = agent.run(sample_data)
    # print("Recommendations:")
    # for i, recommendation in enumerate(response.content.recommendations, 1):
    #     print(f"\nRecommendation {i}:")
    #     print(f"Hospital ID: {recommendation.hospital_id}")
    #     print(f"Hospital Name: {recommendation.hospital_name}")
